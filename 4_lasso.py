...
def extract_selected_features(selected_features):
    ...
    # Determine columns to keep: PATNO + selected_features + MCI
    columns_to_keep = [subject_col] + selected_features + [label_col]
...
def main():
    ...
    subject_col = 'PATNO'
    label_col = 'MCI'
    feature_cols = [col for col in train_scaled.columns if col not in [subject_col, label_col]]
    
    X_train_scaled = train_scaled[feature_cols]
    y_train = train_scaled[label_col]
    subjects_train = train_scaled[subject_col].values
