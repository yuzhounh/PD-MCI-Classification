...
def plot_combined_feature_importance():
    ...
    # Check if SVM data contains 'SVM_Weights' column
    has_svm_weights = 'SVM_Weights' in svm_data.columns
    print(f"SVM_Weights column found: {has_svm_weights}")
    
    # Define algorithm and method correspondence based on SVM weights availability
    if has_svm_weights:
        # Use first approach - with SVM_Weights
        algorithms = {
            'LR': {
                'data': lr_data,
        ...
        # Use second approach - without SVM_Weights (hide first column for SVM)
        algorithms = {
            'LR': {
                'data': lr_data,
                'methods': ['Coefficients', 'SHAP', 'Permutation']
            ...
            'SVM': {
                'data': svm_data,
                'methods': [None, 'SHAP', 'Permutation']  # First method is None to leave empty
            },
            'RF': {
                'data': rf_data,
                'methods': ['Impurity_Importance', 'SHAP', 'Permutation']
    ...
    for i, (alg_name, alg_info) in enumerate(algorithms.items()):
        ...
        for j, method in enumerate(methods):
            ...
            # Skip if method is None (for SVM first column when no SVM_Weights)
            if method is None:
            ...
            for k, bar in enumerate(bars):
                if sorted_values[k] < 0:
                    bar.set_color('#FF0051')  # 255, 0, 81 - Red for negative values
                else:
                    bar.set_color('#008BFB')  # 0, 139, 251 - Blue for positive values
    ...
    # plt.savefig('6_combined_feature_importance_grid.png', dpi=300, bbox_inches='tight')
...
def create_summary_table():
    ...
    for feature in lr_data.index:
        row = {
            ...
            'SVM_Weights': svm_data.loc[feature, 'SVM_Weights'] if has_svm_weights else None,
